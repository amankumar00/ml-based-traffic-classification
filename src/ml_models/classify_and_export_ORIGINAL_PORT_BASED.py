...
def main():
    ...
    # Count classification methods (TESTING: All flows now use ML)
    ...
    ml_classified = len(results)  # All flows use ML now

    # Save to CSV
    results_df.to_csv(output_file, index=False)
    print(f"\n✓ Results saved to {output_file}")
    print(f"  Total flows: {len(results_df)}")
    print(f"  🧪 TESTING MODE: PURE ML CLASSIFICATION (port-based override disabled)")
    print(f"  ML-based classification: {ml_classified} flows (100%)")
