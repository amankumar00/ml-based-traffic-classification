...
def generate_traffic(net, traffic_type='mixed'):
    ...
    if traffic_type == 'video' or traffic_type == 'mixed':
        ...
        if len(hosts) >= 2:
            ...
            # Reduced from 200MB to 5MB for lighter traffic and better energy savings demo
            info('*** Creating VIDEO files and starting servers on h1, h2...\n')
            hosts[0].cmd('dd if=/dev/zero of=/tmp/video1 bs=1M count=5 2>/dev/null &')  # h1
            hosts[1].cmd('dd if=/dev/zero of=/tmp/video2 bs=1M count=5 2>/dev/null &')  # h2
            time.sleep(2)
    ...
    if traffic_type == 'ftp' or traffic_type == 'mixed':
        ...
        if len(hosts) >= 4:
            ...
            # Reduced from 150KB to 50KB for lighter traffic
            hosts[3].cmd('dd if=/dev/urandom of=/tmp/ftpfile1 bs=1024 count=50 2>/dev/null')  # h4
            hosts[3].cmd('while true; do nc -l -p 21 < /tmp/ftpfile1; done > /dev/null 2>&1 &')

            hosts[2].cmd('dd if=/dev/urandom of=/tmp/ftpfile2 bs=1024 count=50 2>/dev/null')  # h3
...
def generate_traffic_phased(net, duration=60):
    """
    Generate traffic in phases to demonstrate FPLF adaptive behavior.

    Phase 1 (0-20s): Light traffic - HTTP + SSH only
    Phase 2 (20-40s): Medium traffic - Add FTP
    Phase 3 (40-60s): Heavy traffic - Add VIDEO

    This creates temporal variation showing FPLF adapting to changing load.
    """
    hosts = net.hosts
    if len(hosts) < 9:
        info('*** Warning: Expected 9 hosts for optimal traffic generation\n')

    phase_duration = duration // 3  # Split into 3 equal phases

    info('=' * 80 + '\n')
    info('*** PHASED TRAFFIC GENERATION (demonstrating FPLF adaptive behavior)\n')
    info('=' * 80 + '\n')

    # ========== PHASE 1: LIGHT TRAFFIC (HTTP + SSH) ==========
    info(f'\n*** PHASE 1 (0-{phase_duration}s): LIGHT TRAFFIC - HTTP + SSH only\n')
    info('*** Expected: 2-3 active links, 30-40% energy savings\n')

    # Start HTTP servers
    if len(hosts) >= 2:
        info('*** Starting HTTP servers on h1, h2...\n')
        hosts[0].cmd('python3 -m http.server 80 > /dev/null 2>&1 &')  # h1
        hosts[1].cmd('python3 -m http.server 8080 > /dev/null 2>&1 &')  # h2
        time.sleep(3)

        # Start HTTP clients
        info('*** Starting HTTP clients: h3->h1:80, h4->h2:8080\n')
        hosts[2].cmd(f'while true; do wget -q -O /dev/null http://{hosts[0].IP()}:80/ 2>/dev/null; sleep 4; done &')
        if len(hosts) >= 4:
            hosts[3].cmd(f'while true; do wget -q -O /dev/null http://{hosts[1].IP()}:8080/ 2>/dev/null; sleep 4; done &')

    # Start SSH servers
    if len(hosts) >= 2:
        info('*** Starting SSH servers on h1, h2...\n')
        hosts[0].cmd('dd if=/dev/urandom of=/tmp/sshdata1 bs=1024 count=100 2>/dev/null')
        hosts[0].cmd('while true; do nc -l -p 22 < /tmp/sshdata1; done > /dev/null 2>&1 &')
        hosts[1].cmd('dd if=/dev/urandom of=/tmp/sshdata2 bs=1024 count=100 2>/dev/null')
        hosts[1].cmd('while true; do nc -l -p 22 < /tmp/sshdata2; done > /dev/null 2>&1 &')
        time.sleep(1)

        # Start SSH clients
        info('*** Starting SSH clients: h5->h1:22, h6->h2:22\n')
        if len(hosts) >= 5:
            hosts[4].cmd(f'while true; do nc {hosts[0].IP()} 22 > /dev/null; sleep 3; done 2>/dev/null &')
        if len(hosts) >= 6:
            hosts[5].cmd(f'while true; do nc {hosts[1].IP()} 22 > /dev/null; sleep 3; done 2>/dev/null &')

    info(f'*** Phase 1 running... (waiting {phase_duration}s)\n')
    time.sleep(phase_duration)

    # ========== PHASE 2: MEDIUM TRAFFIC (ADD FTP) ==========
    info(f'\n*** PHASE 2 ({phase_duration}-{phase_duration*2}s): MEDIUM TRAFFIC - Adding FTP\n')
    info('*** Expected: 4-5 active links, 15-25% energy savings\n')

    if len(hosts) >= 4:
        info('*** Starting FTP servers on h3, h4...\n')
        hosts[2].cmd('dd if=/dev/urandom of=/tmp/ftpfile2 bs=1024 count=50 2>/dev/null')
        hosts[2].cmd('while true; do nc -l -p 21 < /tmp/ftpfile2; done > /dev/null 2>&1 &')
        hosts[3].cmd('dd if=/dev/urandom of=/tmp/ftpfile1 bs=1024 count=50 2>/dev/null')
        hosts[3].cmd('while true; do nc -l -p 21 < /tmp/ftpfile1; done > /dev/null 2>&1 &')
        time.sleep(1)

        # Start FTP clients
        info('*** Starting FTP clients: h6->h4:21, h5->h3:21\n')
        if len(hosts) >= 6:
            hosts[5].cmd(f'while true; do nc {hosts[3].IP()} 21 > /dev/null; sleep 5; done 2>/dev/null &')
        if len(hosts) >= 5:
            hosts[4].cmd(f'while true; do nc {hosts[2].IP()} 21 > /dev/null; sleep 5; done 2>/dev/null &')

    info(f'*** Phase 2 running... (waiting {phase_duration}s)\n')
    time.sleep(phase_duration)

    # ========== PHASE 3: HEAVY TRAFFIC (ADD VIDEO) ==========
    info(f'\n*** PHASE 3 ({phase_duration*2}-{duration}s): HEAVY TRAFFIC - Adding VIDEO\n')
    info('*** Expected: 5-6 active links, 0-10% energy savings\n')

    if len(hosts) >= 2:
        info('*** Creating VIDEO files and starting servers on h1, h2...\n')
        hosts[0].cmd('dd if=/dev/zero of=/tmp/video1 bs=1M count=5 2>/dev/null &')
        hosts[1].cmd('dd if=/dev/zero of=/tmp/video2 bs=1M count=5 2>/dev/null &')
        time.sleep(2)
        ...
        # Start VIDEO clients
        info('*** Starting VIDEO clients: h3->h1:5004, h4->h2:5006\n')
        hosts[2].cmd(f'while true; do nc {hosts[0].IP()} 5004 > /dev/null 2>&1; done &')
        if len(hosts) >= 4:
            hosts[3].cmd(f'while true; do nc {hosts[1].IP()} 5006 > /dev/null 2>&1; done &')
        time.sleep(1)

    remaining_time = duration - (phase_duration * 2)
    info(f'*** Phase 3 running... (waiting {remaining_time}s)\n')
    time.sleep(remaining_time)

    info('\n' + '=' * 80 + '\n')
    info('*** PHASED TRAFFIC COMPLETE\n')
    info('*** Traffic flows created (MATCHING ML CLASSIFIER):\n')
    info('    - HTTP: h3->h1:80, h4->h2:8080 (priority 2) [Phase 1: 0-60s]\n')
    info('    - SSH: h5->h1:22, h6->h2:22 (priority 3) [Phase 1: 0-60s]\n')
    info('    - FTP: h6->h4:21, h5->h3:21 (priority 1) [Phase 2: 20-60s]\n')
    info('    - VIDEO: h3->h1:5004, h4->h2:5006 (priority 4) [Phase 3: 40-60s]\n')
    info('=' * 80 + '\n')
...
def main():
    ...
    parser.add_argument('--traffic', '-g',
                       default=None,
                       choices=['http', 'ftp', 'ssh', 'icmp', 'iperf', 'mixed', 'phased'],
                       help='Generate traffic automatically (use "phased" for temporal variation)')
    ...
    try:
        ...
        if args.traffic:
            if args.traffic == 'phased':
                # Use phased traffic generation for temporal variation
                generate_traffic_phased(net, duration=args.duration)
            else:
                # Use original traffic generation (all at once)
